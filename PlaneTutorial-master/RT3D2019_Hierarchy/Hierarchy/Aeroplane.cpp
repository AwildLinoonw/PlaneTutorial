...
void Aeroplane::Update(bool bPlayerControl)
{
	...
	if(bPlayerControl && m_fSpeed >= 0.5f)
	{
		...
		else if (!Application::s_pApp->IsKeyPressed('A'))
		{
			if(m_v4Rot.x >= 0.0f)
			{
				m_v4Rot.x -= 2.f;
		...
		else if (!Application::s_pApp->IsKeyPressed('Q'))
		{
			if (m_v4Rot.x <= 0.0f)
			{
				m_v4Rot.x += 2.f;
		...
		else if (!Application::s_pApp->IsKeyPressed('O'))
		{
			if (m_v4Rot.z <= 0.0f)
			{
				m_v4Rot.z += 3.f;
